...
from ResourceAllocation.modules.mixers.hypernetwork import HyperNetwork
...
class SubMixer(nn.Module):
    def __init__(self, agent_index, agent_nbrhood, args):
        ...
        self.input_size = tohp.request_num * len(self.agent_nbrhood)
        self.output_size = 1
    ...
    def forward(self, utilities, states):
        utilities = th.reshape(utilities, shape=(utilities.shape[0], utilities.shape[1], utilities.shape[2]*utilities.shape[3]))
        # Now flatten the last 2 dimensions for the network
        utilities = th.reshape(utilities, shape=( *utilities.shape[:2], self.input_size))

        if self.use_hyper_network:  # True
