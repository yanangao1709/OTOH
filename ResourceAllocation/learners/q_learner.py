...
from ResourceAllocation.modules.mixers.lomaq import LocalQMixer
from ResourceAllocation.modules.mixers.gcn.gcn_lomaq import GraphQMixer
...
class QLearner:
    def __init__(self, mac, scheme, args):
        ...
        if args.mixer is not None:
            if args.mixer == "lomaq":
                if getattr(args, "use_gcn", False): # use_gcn = False
    ...
    def compute_local_loss(self, rewards, terminated, mask, target_max_qvals, chosen_action_qvals):
        ...
        for agent_index in range(self.args.n_agents):
            ...
            # Calculate 1-step Q-Learning targets
            targets = local_rewards + self.args.gamma * (1 - local_terminated) * target_max_local_qvals.squeeze(-1)

            # Td-error
            td_error = (chosen_action_local_qvals.squeeze(-1) - targets.detach())
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        chosen_action_qvals = th.gather(mac_out[:, :-1], dim=4, index=actions.unsqueeze(-1)).squeeze(4)  # Remove the last dim
        ...
        if self.args.double_q:
            ...
            target_max_qvals = th.gather(target_mac_out, 4, cur_max_actions).squeeze(4)
        else:
        ...
        # Compute Loss # local_observer = True
        if self.args.local_observer:
        ...
        if getattr(self.args, "monotonicity_method", "weights") == "regularization":
            ...
            print("regularizing_loss", reg_loss.item(), t_env)
