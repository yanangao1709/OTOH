...
def train_decomposer(decomposer, batch, reward_optimizer):
    ...
    # local_rewards_normal = nn.functional.normalize(local_rewards, dim=2)
...
def build_reward_data(decomposer, batch, include_last=True):
    ...
    inputs = batch["obs"][:, :, :, :]
    local_rewards_normal = nn.functional.normalize(batch["reward"], dim=2)
    outputs = local_to_global(local_rewards_normal)
    truth = batch["reward"]
