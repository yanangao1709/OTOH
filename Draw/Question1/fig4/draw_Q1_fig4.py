...
OTOH = pd.read_csv("CRR_times_150.csv")
...
fig = plt.figure()
plt.plot(x, OTOH_episode_reward, alpha=0.5, color='darkorange', label='CRR')
plt.plot(x, avg_OTOH_episode_reward, color='darkorange', marker='s', markevery=1000, label='')
plt.tick_params(labelsize=12)
plt.legend(fontsize=13)
plt.xlabel('Episodes', fontsize=15)
plt.ylabel("CRR used times in an episode", fontsize=15)
plt.grid()
plt.show()